...
export function EligibilityPage({ setCurrentView }: EligibilityPageProps) {
  ...
  return (
    <div ref={containerRef} className="relative z-10 min-h-screen">
      {/* Header */}
      <header className="sticky top-0 z-50 backdrop-blur-2xl bg-white/70 border-b border-white/60">
      ...
      <div className="max-w-4xl mx-auto px-6 py-12">
                ...
                <button
                  ...
                  className={`relative flex items-center justify-center size-12 rounded-full font-semibold text-sm transition-all duration-300 ${
                    step === currentStep
                      ...
                      : "bg-white/70 text-slate-400 border border-white/60"
                  }`}
                >
        ...
        <div className="min-h-[400px]">
          ...
          {currentStep === 1 && (
            ...
            >
              ...
              <div className="rounded-3xl border border-white/60 bg-gradient-to-br from-white/80 via-white/60 to-indigo-50/40 backdrop-blur-xl shadow-xl p-6 space-y-4">
                {systemRequirements.map((req) => (
                  <div
                    key={req.id}
                    className="flex items-center gap-4 p-4 rounded-2xl bg-white/60 border border-white/60 hover:bg-white/80 transition-colors"
                  >
          ...
          {currentStep === 2 && (
            ...
            >
              ...
              <div className="rounded-3xl border border-white/60 bg-gradient-to-br from-white/80 via-white/60 to-emerald-50/40 backdrop-blur-xl shadow-xl p-6 space-y-4">
                {safetyFeatures.map((feature) => (
                  <div
                    key={feature.id}
                    className="flex items-center gap-4 p-4 rounded-2xl bg-white/60 border border-white/60"
                  >
          ...
          {currentStep === 3 && (
            ...
            >
              ...
              <div className="rounded-3xl border border-white/60 bg-gradient-to-br from-white/80 via-white/60 to-violet-50/40 backdrop-blur-xl shadow-xl p-8">
                {/* Status Card */}
                <div className="flex flex-col items-center text-center mb-8">
                  <div className="size-20 rounded-full bg-gradient-to-br from-amber-100 to-amber-50 flex items-center justify-center mb-4 shadow-lg">
                    {getStatusIcon(eligibilityStatus)}
                  </div>
                  <h3 className="text-xl font-bold text-slate-900">
                    {statusInfo.title}
                  </h3>
                  <p className="text-slate-600 mt-1">{statusInfo.subtitle}</p>
                </div>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="space-y-2">
                      {recommendations.map((rec, index) => (
                        <div
                          key={index}
                          className="flex items-start gap-3 p-3 rounded-xl bg-white/60 border border-white/60"
                        >
        ...
        {/* Bottom Navigation */}
        <div
          className="w-full flex items-center justify-between mt-8 pt-6 border-t border-slate-200"
          data-animate
        >
          <Button
            type="button"
